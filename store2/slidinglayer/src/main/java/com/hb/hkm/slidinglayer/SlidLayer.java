package com.hb.hkm.slidinglayer;
...
public class SlidLayer extends FrameLayout {
    ...
    private boolean useShadowAsPadding = false;
    ...
    public SlidLayer(Context context) {
        this(context, null);
    }

    public SlidLayer(Context context, AttributeSet attrs) {
    ...
    public SlidLayer(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPreviewOffsetDistance = ta.getDimensionPixelOffset(R.styleable.SlidLayer_previewOffsetDistance, INVALID_VALUE);
    ...
    public void useShadowPadding(boolean b) {
        useShadowAsPadding = b;
    }
    ...
    public void setPreviewOffsetFillScreen() {
        mPreviewOffsetDistance = getBottom() - getTop();
        checkPreviewModeConsistency();
        invalidate(getLeft(), getTop(), getRight(), getBottom());
    }
    ...
    private int determineNextStateForDrag(final int scrollX, final int scrollY, final int velocityX,
        ...
        if (allowedDirection() == HORIZONTAL) {
            panelSize = getWidth() + (useShadowAsPadding ? 0 : mShadowSize);
            ...
            panelSize = getHeight() + (useShadowAsPadding ? 0 : mShadowSize);
    ...
    /**
     * Sets the default location where the SlidLayer will appear
     *
    ...
    private int getPaddingFromShadowSize() {
        if (useShadowAsPadding) {
            return mShadowSize;
        } else {
            return 0;
        }
    ...
    /**
     * Handler interface for obtaining updates on the <code>SlidLayer</code>'s state.
     * <code>OnInteractListener</code> allows for external classes to be notified when the <code>SlidLayer</code>
